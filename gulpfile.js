...
gulp.task('build:styles', ['build:styles:copy'], function() {
  return gulp
    .src([paths.bulma + bulmaSassFile, paths.src + mainSassFile])
...
gulp.task('build:styles:copy', function() {
  return gulp
    .src(paths.src + mainSassFile)
    .pipe(concat(globalSassFile))
    .pipe(gulp.dest(paths.dest))
})

gulp.task('clean:styles', function(callback) {
  del([paths.dest + mainSassFile, paths.dest + distCssFile])
  callback()
})
...
gulp.task('clean:scripts', function(callback) {
  del([paths.dest + mainJsFile, paths.dest + distJsFile])
  callback()
})
...
gulp.task('github:commit', ['bump-version', 'changelog', 'github:branch:create'], function() {
  ...
  return gulp.src(files).pipe(git.commit('bump version number ' + gutil.env.v))
})
...
gulp.task('bump-version', ['github:pull:source'], function() {
  return gulp
    ...
    .pipe(bump({ type: gutil.env.type ? gutil.env.type : 'patch' }).on('error', gutil.log))
    .pipe(gulp.dest('./'))
})
